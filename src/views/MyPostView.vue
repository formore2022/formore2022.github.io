<template>
    <ul id="my-post-nav" class="nav mt-2 mt-sm-5 mx-1 mx-sm-5 px-4">
...
const posts = computed(() => {
    if (user?.logged_in) {
        return [reactive({ empty_post: true })].concat(...myPosts.value)
    } else {
...
// 更新通知狀態
onMounted(() => { user.notifications.myPost = false })
